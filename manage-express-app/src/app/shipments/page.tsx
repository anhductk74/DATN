...
export default function ShipmentsOverviewPage({ onNavigate }: ShipmentsOverviewPageProps) {
  ...
  // Order statistics (mock data - thay thế bằng API call thực tế)
  ...
  const fetchAllStatistics = async () => {
    try {
      ...
      // Mock order statistics - thay thế bằng API call thực tế từ OrderApiService
      setOrderStats({
        pending: 15,
        readyForPickup: 8,
        packed: 12,
      ...
      <Row gutter={16} className="mb-6">
        <Col span={12}>
          <Card 
            ...
            onClick={() => handleNavigation('/shipments/order-list')}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center mr-4">
                  <OrderedListOutlined className="text-2xl text-blue-600" />
                </div>
                <div>
                  <Title level={4} className="mb-1">Order List</Title>
                  <Text type="secondary">Danh sách đơn hàng chờ lấy</Text>
                  <div className="mt-2">
                    <Tag color="orange">{orderStats.pending} chờ xử lý</Tag>
                    <Tag color="green">{orderStats.readyForPickup} sẵn sàng</Tag>
                  </div>
                </div>
              </div>
