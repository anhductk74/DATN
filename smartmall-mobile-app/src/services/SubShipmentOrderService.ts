...
class SubShipmentOrderService {
  ...
  async getAll(): Promise<ApiResponse<SubShipmentOrderResponseDto[]>> {
    try {
      const headers = await this.getAuthHeaders();
      const response = await axios.get(`${API_BASE_URL}/api/logistics/sub-shipment-orders`, { headers });
  ...
  async create(dto: SubShipmentOrderRequestDto): Promise<ApiResponse<SubShipmentOrderResponseDto>> {
    try {
      const headers = await this.getAuthHeaders();
      const response = await axios.post(`${API_BASE_URL}/api/logistics/sub-shipment-orders`, dto, { headers });
      return this.handleResponse<SubShipmentOrderResponseDto>(response);
    } catch (error) {
      return this.handleError(error);
    }
  ...
  async delete(id: string): Promise<ApiResponse> {
    try {
      const headers = await this.getAuthHeaders();
      const response = await axios.delete(`${API_BASE_URL}/api/logistics/sub-shipment-orders/${id}`, { headers });
