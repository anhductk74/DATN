...
export default function ShopProductsContent() {
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      // Since backend replaces ALL images on update, we need to send ALL images
      // including existing ones that user wants to keep
      const allImageFiles: File[] = [];
      const newImageCount = values.images?.filter(file => file.originFileObj).length || 0;
      let existingImageCount = 0;
      
      // Process all images in the form
      for (const file of values.images || []) {
        if (file.originFileObj) {
          // This is a new uploaded file - add directly
          allImageFiles.push(file.originFileObj);
        } else if (file.url && file.status === 'done') {
          // This is an existing image that user wants to keep  
          // Need to convert URL back to File object to send to backend
          existingImageCount++;
          try {
            const response = await fetch(file.url);
            const blob = await response.blob();
            const existingFile = new File([blob], file.name || `existing-image-${existingImageCount}.jpg`, {
              type: blob.type || 'image/jpeg'
            });
            allImageFiles.push(existingFile);
          } catch (error) {
            console.warn('Failed to fetch existing image:', file.url, error);
      ...
      console.log('Total images to send:', allImageFiles.length);
      console.log('- New images:', newImageCount);
      console.log('- Existing images converted to files:', existingImageCount);
      console.log('Removed variant IDs:', removedVariantIds);

      if (editingProduct && editingProduct.id) {
        // Update product - send ALL images (existing + new) as files
        // This ensures all images user wants are preserved
        await productService.updateProduct(
          editingProduct.id, 
          productData,
          allImageFiles.length > 0 ? allImageFiles : undefined
        );
        
        const totalMsg = allImageFiles.length > 0 ? ` with ${allImageFiles.length} image(s)` : '';
        const detailMsg = existingImageCount > 0 && newImageCount > 0 
          ? ` (${existingImageCount} kept + ${newImageCount} new)`
          : existingImageCount > 0 
            ? ` (${existingImageCount} kept)`
            : newImageCount > 0 
              ? ` (${newImageCount} new)`
              : '';
        message.success(`Product updated${totalMsg}${detailMsg}`);
      } else {
        // Create product - use allImageFiles for consistency
        if (allImageFiles.length > 0) {
          await productService.createProduct(productData, allImageFiles);
        } else {
          await productService.createProductSimple(productData);
        }
        const imgMsg = allImageFiles.length > 0 ? ` with ${allImageFiles.length} image(s)` : '';
        message.success(`Product created successfully${imgMsg}`);
      }
